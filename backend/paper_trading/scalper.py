...
#: controls whether the script runs and which candles it reads.
...
class Scalper:
    ...
    def _run_tick_locked(self) -> dict[str, Any]:
        ...
        if portfolio.get("trading_halted"):
            ...
            self._enter_positions(params, watchlist, source, counters, playbook)
    ...
    def _enter_positions(
        ...
        anti-churn cooldown, then data guards and the entry rule.
        ...
        ``watchlist`` before this method ran.
        """
        ...
        for symbol in watchlist:
            ...
            try:
                entered = self._scan_and_enter_symbol(
                    params, symbol, source, counters, playbook
                )
            except Exception as exc:  # noqa: BLE001 — per-symbol isolation
                counters["errors"] += 1
    ...
    def _scan_and_enter_symbol(
        ...
        return self._enter_one(
            params, symbol, source, direction, values, counters, playbook
        )
    ...
    def _enter_one(
        ...
        """Size and submit one scalp entry with fixed-percent SL/TP attached.
        ...
        per-scalp fraction cap). The latest cached close is re-read
        immediately before sizing (the engine fills market orders at that
        price) so the stops sit at the intended percentages of the actual
        fill basis.
        ...
        if direction == "long":
            side = "buy"
            stop_loss = price * (1.0 - params.sl_pct)
            take_profit = price * (1.0 + params.tp_pct)
        else:
            side = "sell"
            stop_loss = price * (1.0 + params.sl_pct)
            take_profit = price * (1.0 - params.tp_pct)
        ...
        self._log(
            ...
            detail={
                ...
                "vwap_side": vwap_side,
                "ema_state": ema_state,
                "tp_pct": _json_num(params.tp_pct),
                "sl_pct": _json_num(params.sl_pct),
    ...
    def tune_with_ai(self) -> dict[str, Any]:
        ...
        return {"applied": applied, "reasoning": reasoning}
    ...
    def _entry_explanation(
        ...
        return (
            ...
            f"(+{params.tp_pct:.2%}), stop ${stop_loss:,.2f} "
            f"(-{params.sl_pct:.2%}), time-stop after {params.time_stop_bars} bars."
        )
    ...
    def _log_scalp_exit(
        ...
        if reason == "take_profit":
            why = f"the +{params.tp_pct:.2%} take-profit target was hit"
        elif reason == "stop_loss":
            why = f"the -{params.sl_pct:.2%} protective stop-loss was hit"
