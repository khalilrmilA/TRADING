...
class AutoTrader:
    ...
    def _update_trailing(
        ...
        try:
            ...
            try:
                # Engine snapshots serialize opened_at as an ISO-8601 string;
                # raw DB rows carry epoch ms. Accept both, else the window
                # lookup silently degrades to trailing the LAST close.
                if isinstance(opened_ms, (int, float)):
                    opened_ts = pd.Timestamp(int(opened_ms), unit="ms", tz="UTC")
                else:
                    opened_ts = pd.Timestamp(str(opened_ms))
            ...
            self._log(
                ...
                detail={
                    ...
                    "explanation": (
                        ...
                        f"{'raised' if side == 'long' else 'lowered'} to "
                        f"${float(result.get('stop_loss') or new_stop):,.6g} "
    ...
    def _news_context(
        ...
        try:
            # DB-cached only (no network in the trading cycle): the freshest
            # factual positioning line the derivatives intel run persisted.
            from backend.intel.derivatives import DerivativesAgent

            deriv_row = DerivativesAgent.latest(symbol)
            if deriv_row and deriv_row.get("note"):
                derivatives = str(deriv_row["note"])
        except Exception:  # noqa: BLE001 — the intel layer is optional
            logger.debug("Derivatives intel unavailable for the news context")
        return {
            "symbol": symbol,
    ...
    def _enter_candidate(
        ...
        if price <= 0 or not math.isfinite(price):
            ...
            return
        # The free-cash cap only applies to longs: a buy spends cash, while
        # opening a short CREDITS the proceeds (the engine's own buying-power
        # gate is 95% of equity for both sides). Capping shorts by free cash
        # would size every short to dust once cash is deployed in longs.
        cash_cap = (0.95 * cash) / price if direction == "long" else math.inf
        qty = min(
        ...
        if stop_loss is not None and stop_loss <= 0:
            logger.warning(
                "%s %s: ATR stop-loss %.6g <= 0 dropped — entering unprotected",
                symbol, direction, stop_loss,
            )
            stop_loss = None
        if take_profit is not None and take_profit <= 0:
            logger.warning(
                "%s %s: ATR take-profit %.6g <= 0 dropped — no profit target",
                symbol, direction, take_profit,
            )
            take_profit = None
